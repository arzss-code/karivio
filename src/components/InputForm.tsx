...
import { AlertCircle, ChevronLeft, ChevronRight, Loader2, Sparkles, FileText, Plus, Trash2, ChevronDown } from 'lucide-react';
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between relative">
          ...
          {[1, 2, 3, 4].map((step) => (
            <div
              key={step}
              className={`w-9 h-9 flex items-center justify-center rounded-full text-sm font-semibold transition-all duration-300 border-2 ${step === currentStep
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white shadow-sm overflow-hidden min-h-[500px] flex flex-col transition-all duration-300">
        ...
        <div className={`p-6 sm:p-8 flex-1 transition-opacity duration-300 ${currentStep === 2 ? 'block opacity-100 overflow-y-auto max-h-[60vh]' : 'hidden opacity-0'}`}>
          ...
          <div className="space-y-6">
            ...
            <button type="button" onClick={() => appendEdu({ degree: '', institution: '', year: '', gpa: '', description: '' })} className="flex items-center cursor-pointer gap-2 text-sm font-semibold text-neutral-700 hover:text-neutral-900 bg-neutral-100 px-4 py-2.5 rounded-xl transition-colors">
          ...
          <div className="space-y-6">
            {projFields.map((field, index) => (
              <div key={field.id} className="p-5 border border-neutral-200 rounded-2xl bg-neutral-50/50 relative">
                <button type="button" onClick={() => removeProj(index)} className="cursor-pointer absolute top-4 right-4 text-neutral-400 hover:text-red-500 transition-colors">
                  <Trash2 className="h-4 w-4" />
                </button>
            ...
            <button type="button" onClick={() => appendProj({ name: '', description: '' })} className="flex items-center cursor-pointer gap-2 text-sm font-semibold text-neutral-700 hover:text-neutral-900 bg-neutral-100 px-4 py-2.5 rounded-xl transition-colors">
        ...
        <div className={`p-6 sm:p-8 flex-1 transition-opacity duration-300 ${currentStep === 3 ? 'block opacity-100 overflow-y-auto max-h-[60vh]' : 'hidden opacity-0'}`}>
          ...
          <div className="space-y-6">
            {expFields.map((field, index) => (
              <div key={field.id} className="p-5 border border-neutral-200 rounded-2xl bg-neutral-50/50 relative">
                {index > 0 && (
                  <button type="button" onClick={() => removeExp(index)} className="cursor-pointer absolute top-4 right-4 text-neutral-400 hover:text-red-500 transition-colors">
                    <Trash2 className="h-4 w-4" />
                  </button>
                )}
            ...
            <button type="button" onClick={() => appendExp({ title: '', company: '', duration: '', description: '' })} className="flex w-full items-center cursor-pointer justify-center gap-2 text-sm font-semibold text-neutral-700 border border-dashed border-neutral-300 hover:border-neutral-400 hover:bg-neutral-50 px-4 py-4 rounded-2xl transition-colors">
        ...
        <div className={`p-6 sm:p-8 flex-1 flex flex-col transition-opacity duration-300 ${currentStep === 4 ? 'flex opacity-100' : 'hidden opacity-0'}`}>
          ...
          <div>
            ...
            <div className="relative w-full sm:w-72">
              <select
                {...register('language')}
                className="form-input w-full rounded-xl border border-neutral-200 bg-neutral-50 pl-4 pr-10 py-3 text-sm focus:border-neutral-400 focus:bg-white focus:ring-0 cursor-pointer appearance-none transition-all"
              >
        ...
        <div className="border-t border-neutral-100 p-5 sm:px-8 bg-neutral-50 flex justify-between items-center mt-auto">
          {currentStep > 1 ? (
            <button
              type="button"
              onClick={prevStep}
              className="inline-flex items-center justify-center cursor-pointer rounded-xl px-5 py-2.5 text-sm font-semibold text-neutral-700 bg-white border border-neutral-200 shadow-sm hover:bg-neutral-50 transition-colors"
          ...
          {currentStep < totalSteps ? (
            <button
              type="button"
              onClick={nextStep}
              className="inline-flex items-center cursor-pointer justify-center rounded-xl px-6 py-2.5 text-sm font-semibold text-white bg-neutral-900 border border-transparent shadow-sm hover:bg-neutral-800 transition-colors"
            >
              Next Step
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={state.isGenerating}
                className="inline-flex items-center cursor-pointer justify-center rounded-xl px-5 py-2.5 text-sm font-semibold text-neutral-900 bg-white border border-neutral-200 hover:bg-neutral-50 transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                disabled={state.isGenerating}
                className="inline-flex items-center cursor-pointer justify-center rounded-xl px-6 py-2.5 text-sm font-semibold text-white bg-neutral-900 hover:bg-neutral-800 transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
              >
